...
export default function ProductDetail({ navigation }) {
  ...
  const sendEmailMessage = () => {
    const subject = "Regarding " + product.title.replace(/\s/g, "%20"); // Escape spaces in subject
  ...
  const deleteFromFiresotre = async () => {
    console.log('Deleted');
  ...
  const paymentGateway=()=>{
    ...
    RNUpiPayment.initializePayment(
      {
        vpa: '9336519454@axl', // or can be john@ybl or mobileNo@upi
        payeeName: 'John Doe',
        amount: '1',
  ...
  return (
    <ScrollView className="bg-white" >
      ...
      <View className="p-3">
        ...
        <Text style={{ fontSize: 18, fontWeight: 'bold', marginTop: 1, color: '#333' }}>
          Rs. {product?.price}
        </Text>
